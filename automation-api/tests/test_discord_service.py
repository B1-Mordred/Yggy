...
def test_discord_service_splits_long_bot_messages(monkeypatch):
    monkeypatch.setenv("DISCORD_DRY_RUN", "false")
    monkeypatch.setenv("DISCORD_BOT_TOKEN", "test-token")
    monkeypatch.setenv("DISCORD_CHANNEL_BRIEFINGS", "12345")
    monkeypatch.delenv("DISCORD_WEBHOOK_BRIEFINGS", raising=False)
    ...
    result = DiscordService().send("briefings", ("alpha " * 500).strip())

    assert result["sent"] is True
    assert result["transport"] == "bot"
    assert result["message_count"] == len(calls)
    assert len(calls) > 1
    assert all(len(call["json"]["content"]) <= 2000 for call in calls)


def test_discord_service_splits_long_webhook_messages(monkeypatch):
    monkeypatch.setenv("DISCORD_DRY_RUN", "false")
    monkeypatch.setenv("DISCORD_WEBHOOK_BRIEFINGS", "https://discord.com/api/webhooks/test/webhook")
    ...
    result = DiscordService().send("briefings", ("beta " * 500).strip())
